...
const ROTATION_ANGLE_MAX: float = 8.0

# For idle wobblec
const IDLE_ROTATION_SPEED: float = 0.56
const IDLE_ROTATION_MAGNITUDE: float = 1.5
var idle_rotation_seed: float = randf() * 100.0
...
func add_light_effect():
	...
	light_effect.texture = preload("res://game/assets/soft_light.png")
...
func _process(delta):
	...
	if not is_dragging and not is_working:
		...
		_update_perspective_rotation(delta)
...
func setup_interaction_indicator():
	...
	indicator_sprite.texture = preload("res://game/assets/interaction_indicator.png")
...
func play_pop_animation():
	...
	tween.tween_property(self, "scale", Vector2(1.2, 1.2), 0.4).from(Vector2.ONE)
	tween.tween_property(self, "rotation_degrees", 5.0, 0.5).from(0.0).set_trans(Tween.TRANS_SPRING)
	tween.chain().tween_property(self, "scale", Vector2.ONE, 0.3)
	tween.chain().tween_property(self, "rotation_degrees", 0.0, 0.3)


func _update_perspective_rotation(delta):
	...
	if is_hovered and not is_dragging:
		...
		var card_size = collision_shape.shape.size

		# Map mouse position from (-size/2, +size/2) to (-1, 1)
		var input_x = remap(mouse_pos.x, -card_size.x / 2.0, card_size.x / 2.0, -1.0, 1.0)
		var input_y = remap(mouse_pos.y, -card_size.y / 2.0, card_size.y / 2.0, -1.0, 1.0)
		
		target_rot_x = -input_y * ROTATION_ANGLE_MAX
		target_rot_y = input_x * ROTATION_ANGLE_MAX
	else:
		# Idle Wobble
		var time = Time.get_ticks_msec() / 200 # Get time in seconds
		target_rot_x = sin(time * IDLE_ROTATION_SPEED + idle_rotation_seed) * IDLE_ROTATION_MAGNITUDE
		target_rot_y = cos(time * IDLE_ROTATION_SPEED * 0.7 + idle_rotation_seed) * IDLE_ROTATION_MAGNITUDE

	# Apply the rotation smoothly (Lerp)
	var mat : ShaderMaterial = material.duplicate()
	material = mat
	%Sprite2D.material = mat
	%ImageRect.material = mat
	mat.set_shader_parameter("x_rot", lerp(mat.get_shader_parameter("x_rot"), target_rot_x, ROTATION_SPEED))
	mat.set_shader_parameter("y_rot", lerp(mat.get_shader_parameter("y_rot"), target_rot_y, ROTATION_SPEED))
